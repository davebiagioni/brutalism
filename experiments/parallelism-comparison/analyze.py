#!/usr/bin/env python3
"""Aggregate metrics from results/runs.json, including held-out correctness."""
...
def main() -> int:
    ...
    def row(label, getter):
        cells = [f"{agg([getter(r) for r in by_arm[a]]):>{CELL_W}}" for a in arms]
        print(f"{label:<32} | " + " | ".join(cells))

    row("visible pass rate",
        lambda r: r["grading"]["visible"]["passed"] / max(r["grading"]["visible"]["total"], 1))
    row("visible passed",
        lambda r: r["grading"]["visible"]["passed"])
    row("HOLDOUT pass rate",
        lambda r: r["grading"]["holdout"]["passed"] / max(r["grading"]["holdout"]["total"], 1))
    row("HOLDOUT passed (of total)",
        lambda r: r["grading"]["holdout"]["passed"])
    ...
    row("impl non-blank lines",
        lambda r: sum(c["non_blank_lines"] for c in r.get("code", {}).values()) or None)
    row("impl chars", lambda r: sum(c["chars"] for c in r.get("code", {}).values()) or None)

    print()
    print("tool call breakdown (mean per run):")
    for a in arms:
        tots: Counter[str] = Counter()
        n = len(by_arm[a])
        for r in by_arm[a]:
            for k, v in (r.get("metrics", {}).get("tool_calls_by_name") or {}).items():
                tots[k] += v
        items = ", ".join(f"{k}={v / n:.1f}" for k, v in tots.most_common())
        print(f"  {a}: {items or '(none)'}")

    print()
    print("held-out tests most often missed (failure count across runs):")
    for a in arms:
        fail_counter: Counter[str] = Counter()
        for r in by_arm[a]:
            for name in r.get("grading", {}).get("holdout", {}).get("failed", []):
                fail_counter[name] += 1
        if not fail_counter:
            print(f"  {a}: (none)")
            continue
        items = ", ".join(f"{n}({c})" for n, c in fail_counter.most_common(8))
        print(f"  {a}: {items}")
